...
//
//*****************************************************************************
static void ScheduleNextTransmission(tUSBBuffer *psBuffer) {
    ...
    if (ui32Packet) {
        ...
        if (ui32Space) {
            ...
            psBuffer->pfnTransfer(psBuffer->pvHandle,
                                  (psBuffer->sPrivateData.sRingBuf.pui8Buf + psBuffer->sPrivateData.sRingBuf.ui32ReadIndex),
                                  ui32Space,
                                  (((ui32Space < ui32Packet) && (ui32Space < ui32Total)) ? false : true));
            ...
            if ((ui32Space < ui32Packet) && (ui32Space < ui32Total)) {
                ...
                psBuffer->pfnTransfer(psBuffer->pvHandle, psBuffer->sPrivateData.sRingBuf.pui8Buf, ui32Space, true);
            }
        } else {
            //
            ...
            if (psBuffer->sPrivateData.ui32LastSent == ui32Packet) {
                ...
                if (psBuffer->sPrivateData.ui32Flags & USB_BUFFER_FLAG_SEND_ZLP) {
                    ...
                    psBuffer->pfnTransfer(psBuffer->pvHandle, psBuffer->sPrivateData.sRingBuf.pui8Buf, 0, true);
                }
            }
        }
...
//
//*****************************************************************************
static uint32_t HandleRxAvailable(tUSBBuffer *psBuffer, uint32_t ui32Size, uint8_t *pui8Data) {
    ...
    if (pui8Data) {
        ...
        //
        if ((pui8Data >= psBuffer->pui8Buffer) && (pui8Data < psBuffer->pui8Buffer + psBuffer->ui32BufferSize)) {
            //
            ...
            //
            USBRingBufWrite(&psBuffer->sPrivateData.sRingBuf, pui8Data, ui32Read);
        ...
        ui32Read = psBuffer->pfnTransfer(psBuffer->pvHandle,
                                         (psBuffer->sPrivateData.sRingBuf.pui8Buf + psBuffer->sPrivateData.sRingBuf.ui32WriteIndex),
                                         ui32Avail,
                                         true);
        ...
        if (ui32Read < ui32Packet) {
            ...
            if (ui32Avail) {
                ...
                //
                if (ui32Packet) {
                    USBRingBufAdvanceWrite(&psBuffer->sPrivateData.sRingBuf, ui32Packet);
                }
            }
        }
...
//
//*****************************************************************************
static uint32_t HandleDataRemaining(tUSBBuffer *psBuffer) {
    uint32_t ui32BufData, ui32ClientData;

    //
    // How much data does the client currently have buffered?
    //
    ui32ClientData = psBuffer->pfnCallback(psBuffer->pvCBData, USB_EVENT_DATA_REMAINING, 0, (void *)0);
...
//
//*****************************************************************************
static uint32_t HandleTxComplete(tUSBBuffer *psBuffer, uint32_t ui32Size) {
    //
...
//
//*****************************************************************************
static uint32_t HandleRequestBuffer(tUSBBuffer *psBuffer, uint32_t ui32Size, uint8_t **ppui8Buffer) {
    uint32_t ui32Space;
    ...
    if (ui32Space >= ui32Size) {
        ...
        //
        *ppui8Buffer = psBuffer->sPrivateData.sRingBuf.pui8Buf + psBuffer->sPrivateData.sRingBuf.ui32WriteIndex;
        return (ui32Size);
    } else {
        //
...
const tUSBBuffer *USBBufferInit(tUSBBuffer *psBuffer) {
    ...
    //
    ASSERT(psBuffer && psBuffer->pui8Buffer && psBuffer->ui32BufferSize && psBuffer->pfnAvailable && psBuffer->pfnTransfer &&
    ...
    USBRingBufInit(&psBuffer->sPrivateData.sRingBuf, psBuffer->pui8Buffer, psBuffer->ui32BufferSize);
...
uint32_t USBBufferEventCallback(void *pvCBData, uint32_t ui32Event, uint32_t ui32MsgValue, void *pvMsgData) {
    ...
    switch (ui32Event) {
        ...
        case USB_EVENT_RX_AVAILABLE: {
            ...
            if (!psBuffer->bTransmitBuffer) {
                return (HandleRxAvailable(psBuffer, ui32MsgValue, (uint8_t *)pvMsgData));
            }
            break;
        }
        ...
        case USB_EVENT_REQUEST_BUFFER: {
            ...
            //
            if (!psBuffer->bTransmitBuffer) {
                return (HandleRequestBuffer(psBuffer, ui32MsgValue, (uint8_t **)pvMsgData));
            }
            break;
        }
    ...
    //
    return (psBuffer->pfnCallback(psBuffer->pvCBData, ui32Event, ui32MsgValue, pvMsgData));
}
