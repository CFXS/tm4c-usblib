...
static void ClearIdleTimers(const tUSBDHIDDevice *psHIDDevice) {
    ...
    //
    for (ui32Loop = 0; ui32Loop < psHIDDevice->ui8NumInputReports; ui32Loop++) {
        psHIDDevice->psReportIdle[ui32Loop].ui16TimeTillNextmS = psHIDDevice->psReportIdle[ui32Loop].ui8Duration4mS * 4;
    }
}
...
static void ProcessIdleTimers(const tUSBDHIDDevice *psHIDDevice, uint32_t ui32ElapsedmS) {
    ...
    for (ui32Loop = 0; ui32Loop < psHIDDevice->ui8NumInputReports; ui32Loop++) {
        ...
        //
        psHIDDevice->psReportIdle[ui32Loop].ui32TimeSinceReportmS += ui32ElapsedmS;
        ...
        if (psHIDDevice->psReportIdle[ui32Loop].ui8Duration4mS) {
            ...
            //
            if (psHIDDevice->psReportIdle[ui32Loop].ui16TimeTillNextmS <= ui32ElapsedmS) {
                //
                // The timer is about to expire.  Can we send a report right
                // now?
                //
                if ((psInst->iHIDTxState == eHIDStateIdle) && (psInst->bSendInProgress == false)) {
                    //
                    ...
                    ui32SizeReport = psHIDDevice->pfnRxCallback(
                        psHIDDevice->pvRxCBData, USBD_HID_EVENT_IDLE_TIMEOUT, psHIDDevice->psReportIdle[ui32Loop].ui8ReportID, &pvReport);

                    //
                    // Schedule the report for transmission.
                    //
                    USBDHIDReportWrite((void *)psHIDDevice, (uint8_t *)pvReport, ui32SizeReport, true);
                ...
                //
                psHIDDevice->psReportIdle[ui32Loop].ui16TimeTillNextmS -= ui32ElapsedmS;
            }
        }
    }
    ...
    //
    SetDeferredOpFlag(&psInst->ui16DeferredOpFlags, HID_DO_SEND_IDLE_REPORT, bDeferred);
}
    ...
    for (ui32Loop = 0; ui32Loop < psHIDDevice->ui8NumInputReports; ui32Loop++) {
        ...
        if (!ui8ReportID || (ui8ReportID == psIdle->ui8ReportID)) {
            ...
            if (ui8Timeout4mS) {
                ...
                //
                if (psIdle->ui32TimeSinceReportmS >= ((uint32_t)ui8Timeout4mS * 4)) {
                    psIdle->ui16TimeTillNextmS = 0;
                    bReportNeeded              = true;
                } else {
                    psIdle->ui16TimeTillNextmS = (((uint16_t)ui8Timeout4mS * 4) - psIdle->ui32TimeSinceReportmS);
                }
            }
        }
    }
...
//
//*****************************************************************************
static uint32_t GetIdleTimeout(const tUSBDHIDDevice *psHIDDevice, uint8_t ui8ReportID) {
    uint32_t ui32Loop;
    tHIDReportIdle *psIdle;
...
//
//*****************************************************************************
static uint32_t FindHIDDescriptor(const tUSBDHIDDevice *psHIDDevice, uint8_t ui8Type, uint32_t ui32Index, uint32_t *pui32Len) {
    bool bFoundType;
    ...
    //
    for (ui32Loop = 0; ui32Loop < psHIDDevice->psHIDDescriptor->bNumDescriptors; ui32Loop++) {
    ...
    if ((ui8Type == USB_HID_DTYPE_PHYSICAL) && bFoundType) {
        ...
        //
        psDesc    = &(psHIDDevice->psHIDDescriptor->sClassDescriptor[ui32LastFound]);
        *pui32Len = (uint32_t)psDesc->wDescriptorLength;
...
static int32_t ScheduleReportTransmission(tHIDInstance *psHIDInst) {
    ...
    //
    ui32NumBytes = (uint32_t)(psHIDInst->ui16InReportSize - psHIDInst->ui16InReportIndex);
    ...
    //
    i32Retcode = MAP_USBEndpointDataPut(psHIDInst->ui32USBBase, psHIDInst->ui8INEndpoint, pui8Data, ui32NumBytes);
    ...
    if (i32Retcode != -1) {
        ...
        //
        i32Retcode = MAP_USBEndpointDataSend(psHIDInst->ui32USBBase, psHIDInst->ui8INEndpoint, USB_TRANS_IN);
    }
...
//
//*****************************************************************************
static bool ProcessDataFromHost(tUSBDHIDDevice *psHIDDevice, uint32_t ui32Status) {
    ...
    if (ui32EPStatus & USB_DEV_RX_PKT_RDY) {
        ...
        //
        ui32Size = MAP_USBEndpointDataAvail(psInst->ui32USBBase, psInst->ui8OUTEndpoint);
        ...
        //
        psHIDDevice->pfnRxCallback(psHIDDevice->pvRxCBData, USB_EVENT_RX_AVAILABLE, ui32Size, (void *)0);
    } else {
        //
        ...
        if (ui32EPStatus & USB_RX_ERROR_FLAGS) {
            ...
            //
            psHIDDevice->pfnRxCallback(psHIDDevice->pvRxCBData, USB_EVENT_ERROR, (ui32EPStatus & USB_RX_ERROR_FLAGS), (void *)0);
        }
        return (false);
    }
...
//
//*****************************************************************************
static bool ProcessDataToHost(tUSBDHIDDevice *psHIDDevice, uint32_t ui32Status) {
    tHIDInstance *psInst;
    ...
    // Get the endpoint status to see why we were called.
    //
    ui32EPStatus = MAP_USBEndpointStatus(psInst->ui32USBBase, psInst->ui8INEndpoint);

    //
    // Clear the status bits.
    //
    MAP_USBDevEndpointStatusClear(psInst->ui32USBBase, psInst->ui8INEndpoint, ui32EPStatus);
    ...
    if (psInst->ui16InReportSize == psInst->ui16InReportIndex) {
        ...
        //
        psHIDDevice->pfnTxCallback(psHIDDevice->pvTxCBData, USB_EVENT_TX_COMPLETE, psInst->ui16InReportSize, (void *)0);
...
static void HandleEndpoints(void *pvHIDInstance, uint32_t ui32Status) {
    ...
    if (ui32Status & (0x10000 << USBEPToIndex(psInst->ui8OUTEndpoint))) {
        ...
        //
        ProcessDataFromHost((tUSBDHIDDevice *)pvHIDInstance, ui32Status);
    }
    ...
    if (ui32Status & (1 << USBEPToIndex(psInst->ui8INEndpoint))) {
        ProcessDataToHost((tUSBDHIDDevice *)pvHIDInstance, ui32Status);
    }
}
...
static void HandleConfigChange(void *pvHIDInstance, uint32_t ui32Info) {
    ...
    if (!psInst->bConnected) {
        ...
        //
        psHIDDevice->pfnRxCallback(psHIDDevice->pvRxCBData, USB_EVENT_CONNECTED, 0, (void *)0);
    }
    ...
    switch (ui32Request) {
        ...
        case USB_EVENT_LPM_RESUME: {
            if (psHIDDevice->pfnRxCallback) {
                ...
                //
                psHIDDevice->pfnRxCallback(psHIDDevice->pvRxCBData, USB_EVENT_LPM_RESUME, 0, (void *)0);
            }
            break;
        }
        case USB_EVENT_LPM_SLEEP: {
            if (psHIDDevice->pfnRxCallback) {
                ...
                //
                psHIDDevice->pfnRxCallback(psHIDDevice->pvRxCBData, USB_EVENT_LPM_SLEEP, 0, (void *)0);
            }
            break;
        }
        case USB_EVENT_LPM_ERROR: {
            if (psHIDDevice->pfnRxCallback) {
                ...
                //
                psHIDDevice->pfnRxCallback(psHIDDevice->pvRxCBData, USB_EVENT_LPM_ERROR, 0, (void *)0);
            }
            break;
        }
        default: {
...
static void HandleDisconnect(void *pvHIDInstance) {
    ...
    if (psHIDDevice->sPrivateData.bConnected) {
        ...
        //
        psHIDDevice->pfnRxCallback(psHIDDevice->pvRxCBData, USB_EVENT_DISCONNECTED, 0, (void *)0);
    }
...
static void HandleGetDescriptor(void *pvHIDInstance, tUSBRequest *psUSBRequest) {
    ...
    switch (psUSBRequest->wValue >> 8) {
        ...
        case USB_HID_DTYPE_PHYSICAL: {
            ...
            //
            ui32Size = 0;
            ui32Desc = FindHIDDescriptor(psHIDDevice, psUSBRequest->wValue >> 8, psUSBRequest->wValue & 0xFF, &ui32Size);
            ...
            //
            USBDCDSendDataEP0(0, (uint8_t *)psHIDDevice->ppui8ClassDescriptors[ui32Desc], ui32Size);
    ...
    switch (psUSBRequest->bRequest) {
        ...
        case USBREQ_SET_REPORT: {
            ...
            psInst->pui8OutReportData = (uint8_t *)psHIDDevice->pfnRxCallback(
                psHIDDevice->pvRxCBData, USBD_HID_EVENT_GET_REPORT_BUFFER, psUSBRequest->wValue, (void *)(uint32_t)(psUSBRequest->wLength));
            ...
            if (!psInst->pui8OutReportData) {
                ...
                //
                USBDCDRequestDataEP0(0, psInst->pui8OutReportData, (uint32_t)psUSBRequest->wLength);
        ...
        case USBREQ_GET_REPORT: {
            ...
            //
            ui32Size = psHIDDevice->pfnRxCallback(psHIDDevice->pvRxCBData, USBD_HID_EVENT_GET_REPORT, psUSBRequest->wValue, &pui8Report);
        ...
        case USBREQ_SET_PROTOCOL: {
            if (psHIDDevice->ui8Subclass == USB_HID_SCLASS_BOOT) {
                ...
                // application.
                //
                psHIDDevice->pfnRxCallback(psHIDDevice->pvRxCBData, USBD_HID_EVENT_SET_PROTOCOL, psUSBRequest->wValue, (void *)0);
            } else {
                //
        ...
        case USBREQ_GET_PROTOCOL: {
            if (psHIDDevice->ui8Subclass == USB_HID_SCLASS_BOOT) {
                ...
                //
                ui8Protocol = (uint8_t)psHIDDevice->pfnRxCallback(psHIDDevice->pvRxCBData, USBD_HID_EVENT_GET_PROTOCOL, 0, (void *)0);
...
static void HandleEP0DataReceived(void *pvHIDInstance, uint32_t ui32DataSize) {
    ...
    // the client.
    //
    psHIDDevice->pfnRxCallback(psHIDDevice->pvRxCBData, USBD_HID_EVENT_SET_REPORT, psInst->ui16OutReportSize, psInst->pui8OutReportData);
}
...
static void HandleEP0DataSent(void *pvHIDInstance, uint32_t ui32Info) {
    ...
    if (psInst->bGetRequestPending) {
        ...
        psHIDDevice->pfnRxCallback(psHIDDevice->pvRxCBData, USBD_HID_EVENT_REPORT_SENT, 0, (void *)0);
    }
...
static void HandleSuspend(void *pvHIDInstance) {
    ...
    //
    psHIDDevice->pfnRxCallback(psHIDDevice->pvRxCBData, USB_EVENT_SUSPEND, 0, (void *)0);
}
...
static void HandleResume(void *pvHIDInstance) {
    ...
    // Pass the event on to the client.
    //
    psHIDDevice->pfnRxCallback(psHIDDevice->pvRxCBData, USB_EVENT_RESUME, 0, (void *)0);
}
...
void *USBDHIDInit(uint32_t ui32Index, tUSBDHIDDevice *psHIDDevice) {
    ...
    //
    USBDCDInit(ui32Index, &psHIDDevice->sPrivateData.sDevInfo, (void *)psHIDDevice);
...
void *USBDHIDCompositeInit(uint32_t ui32Index, tUSBDHIDDevice *psHIDDevice, tCompositeEntry *psCompEntry) {
    ...
    psInst->sDevInfo.ppui8StringDescriptors   = psHIDDevice->ppui8StringDescriptors;
    psInst->sDevInfo.ui32NumStringDescriptors = psHIDDevice->ui32NumStringDescriptors;
    ...
    //
    psEndpoint = USBDCDConfigGetInterfaceEndpoint(psHIDDevice->ppsConfigDescriptor[0], psInst->ui8Interface, 0, 0);
    ...
    //
    psEndpoint = USBDCDConfigGetInterfaceEndpoint(psHIDDevice->ppsConfigDescriptor[0], psInst->ui8Interface, 0, 1);
    if (psEndpoint) {
        if (psEndpoint->bEndpointAddress & 0x80) {
            psInst->ui8INEndpoint = IndexToUSBEP(psEndpoint->bEndpointAddress);
        } else {
...
uint32_t USBDHIDReportWrite(void *pvHIDInstance, uint8_t *pi8Data, uint32_t ui32Length, bool bLast) {
    ...
    //
    if (ui32Length) {
        ClearReportTimer((tUSBDHIDDevice *)pvHIDInstance, *pi8Data);
    }
...
uint32_t USBDHIDPacketRead(void *pvHIDInstance, uint8_t *pi8Data, uint32_t ui32Length, bool bLast) {
    ...
    //
    ui32EPStatus = MAP_USBEndpointStatus(psInst->ui32USBBase, psInst->ui8OUTEndpoint);
    if (ui32EPStatus & USB_DEV_RX_PKT_RDY) {
        //
        // How many bytes are available for us to receive?
        //
        ui32Pkt = MAP_USBEndpointDataAvail(psInst->ui32USBBase, psInst->ui8OUTEndpoint);

        //
        // Get as much data as we can.
        //
        ui32Count  = ui32Length;
        i32Retcode = MAP_USBEndpointDataGet(psInst->ui32USBBase, psInst->ui8OUTEndpoint, pi8Data, &ui32Count);
        ...
        if (ui32Count == ui32Pkt) {
            ...
            // waiting.
            //
            MAP_USBDevEndpointStatusClear(psInst->ui32USBBase, psInst->ui8OUTEndpoint, ui32EPStatus);

            //
            // Acknowledge the data, thus freeing the host to send the
            // next packet.
            //
            MAP_USBDevEndpointDataAck(psInst->ui32USBBase, psInst->ui8OUTEndpoint, true);

            //
            // Clear the flag we set to indicate that a packet read is
            // pending.
            //
            SetDeferredOpFlag(&psInst->ui16DeferredOpFlags, HID_DO_PACKET_RX, false);
        }
