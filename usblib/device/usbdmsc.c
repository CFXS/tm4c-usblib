...
//
//*****************************************************************************
const tConfigSection g_sMSCConfigSection = {sizeof(g_pui8MSCDescriptor), g_pui8MSCDescriptor};

const tConfigSection g_sMSCInterfaceSection = {sizeof(g_pui8MSCInterface), g_pui8MSCInterface};
...
//
//*****************************************************************************
const tConfigSection *g_psMSCSections[] = {&g_sMSCConfigSection, &g_sMSCInterfaceSection};

#define NUM_MSC_SECTIONS (sizeof(g_psMSCSections) / sizeof(g_psMSCSections[0]))
...
//
//*****************************************************************************
const tConfigHeader g_sMSCConfigHeader = {NUM_MSC_SECTIONS, g_psMSCSections};
...
//
//*****************************************************************************
const tConfigHeader *const g_ppsMSCConfigDescriptors[] = {&g_sMSCConfigHeader};
...
static void HandleDevice(void *pvMSCDevice, uint32_t ui32Request, void *pvRequestData);
...
static void HandleEndpoints(void *pvMSCDevice, uint32_t ui32Status) {
    ...
    if ((ui32Status & (1 << USBEPToIndex(psInst->ui8INEndpoint))) ||
        ((psInst->ui32Flags & USBD_FLAG_DMA_IN) &&
         (USBLibDMAChannelStatus(psInst->psDMAInstance, psInst->ui8INDMA) & USBLIBSTATUS_DMA_COMPLETE))) {
        switch (psInst->ui8SCSIState) {
            ...
            case STATE_SCSI_SEND_BLOCKS: {
                ...
                if (psInst->ui32BytesToTransfer == 0) {
                    ...
                    //
                    MAP_USBEndpointDMADisable(USB0_BASE, psInst->ui8INEndpoint, USB_EP_DEV_IN);

                    if (psMSCDevice->pfnEventCallback) {
                        psMSCDevice->pfnEventCallback(0, USBD_MSC_EVENT_IDLE, 0, 0);
                    }
                    ...
                    // out the status.
                    //
                    if (USBEndpointStatus(USB0_BASE, psInst->ui8INEndpoint) & USB_DEV_TX_TXPKTRDY) {
                        //
                ...
                //
                if (psMSCDevice->sMediaFunctions.pfnBlockRead(psInst->pvMedia, (uint8_t *)psInst->pui32Buffer, psInst->ui32CurrentLBA, 1) ==
                    0) {
                }
                ...
                //
                USBLibDMATransfer(psInst->psDMAInstance, psInst->ui8INDMA, psInst->pui32Buffer, g_pui32BlockSize);
    ...
    if ((ui32Status & (0x10000 << USBEPToIndex(psInst->ui8OUTEndpoint))) ||
        ((psInst->ui32Flags & USBD_FLAG_DMA_OUT) &&
         (USBLibDMAChannelStatus(psInst->psDMAInstance, psInst->ui8OUTDMA) & USBLIBSTATUS_DMA_COMPLETE))) {
        //
        ...
        switch (psInst->ui8SCSIState) {
            ...
            case STATE_SCSI_RECEIVE_BLOCKS: {
                ...
                // Write the new data.
                //
                psMSCDevice->sMediaFunctions.pfnBlockWrite(psInst->pvMedia, (uint8_t *)psInst->pui32Buffer, psInst->ui32CurrentLBA, 1);
                ...
                if (psInst->ui32BytesToTransfer == 0) {
                    ...
                    // Disable uDMA on the endpoint
                    //
                    MAP_USBEndpointDMADisable(USB0_BASE, psInst->ui8OUTEndpoint, USB_EP_DEV_OUT);
                    ...
                    //
                    USBLibDMATransfer(psInst->psDMAInstance, psInst->ui8OUTDMA, psInst->pui32Buffer, g_pui32BlockSize);
                }
            ...
            case STATE_SCSI_IDLE: {
                ...
                MAP_USBEndpointDataGet(psInst->ui32USBBase, psInst->ui8OUTEndpoint, g_pui8Command, &ui32Size);
                psSCSICBW = (tMSCCBW *)g_pui8Command;

                //
                // Acknowledge the OUT data packet.
                //
                MAP_USBDevEndpointDataAck(psInst->ui32USBBase, psInst->ui8OUTEndpoint, false);
    ...
    switch (ui32Request) {
        ...
        case USB_EVENT_COMP_EP_CHANGE: {
            ...
            if (pui8Data[0] & USB_EP_DESC_IN) {
                ...
                if (psInst->ui8INDMA != 0) {
                    USBLibDMAChannelRelease(psInst->psDMAInstance, psInst->ui8INDMA);
                }
                ...
                //
                psInst->ui8INDMA =
                    USBLibDMAChannelAllocate(psInst->psDMAInstance, psInst->ui8INEndpoint, 0, USB_DMA_EP_TX | USB_DMA_EP_DEVICE);

                //
                // Set the DMA individual transfer size.
                //
                USBLibDMAUnitSizeSet(psInst->psDMAInstance, psInst->ui8INDMA, 32);

                //
                // Set the DMA arbitration size.
                //
                USBLibDMAArbSizeSet(psInst->psDMAInstance, psInst->ui8INDMA, 16);
            } else {
                //
                // If the DMA channel has already been allocated then clear
                // that channel and prepare to possibly use a new one.
                ...
                if (psInst->ui8OUTDMA != 0) {
                    USBLibDMAChannelRelease(psInst->psDMAInstance, psInst->ui8OUTDMA);
                }
                ...
                //
                psInst->ui8OUTDMA =
                    USBLibDMAChannelAllocate(psInst->psDMAInstance, psInst->ui8OUTEndpoint, 0, USB_DMA_EP_RX | USB_DMA_EP_DEVICE);

                //
                // Set the DMA individual transfer size.
                //
                USBLibDMAUnitSizeSet(psInst->psDMAInstance, psInst->ui8OUTDMA, 32);

                //
                // Set the DMA arbitration size.
                //
                USBLibDMAArbSizeSet(psInst->psDMAInstance, psInst->ui8OUTDMA, 16);
            }
            break;
        }
        case USB_EVENT_LPM_RESUME: {
            if (psMSCDevice->pfnEventCallback) {
                ...
                //
                psMSCDevice->pfnEventCallback(0, USB_EVENT_LPM_RESUME, 0, (void *)0);
            }
            break;
        }
...
static void HandleDisconnect(void *pvMSCDevice) {
    ...
    if (psMSCDevice->pfnEventCallback) {
        ...
        //
        psMSCDevice->pfnEventCallback(pvMSCDevice, USB_EVENT_DISCONNECTED, 0, 0);
    }
}
...
static void ConfigChangeHandler(void *pvMSCDevice, uint32_t ui32Value) {
    ...
    if (psMSCDevice->sPrivateData.ui8OUTDMA != 0) {
        USBLibDMAChannelRelease(psMSCDevice->sPrivateData.psDMAInstance, psMSCDevice->sPrivateData.ui8OUTDMA);
    }

    //
    // Configure the DMA for the OUT endpoint.
    //
    psMSCDevice->sPrivateData.ui8OUTDMA = USBLibDMAChannelAllocate(
        psMSCDevice->sPrivateData.psDMAInstance, psMSCDevice->sPrivateData.ui8OUTEndpoint, 64, USB_DMA_EP_RX | USB_DMA_EP_DEVICE);

    USBLibDMAUnitSizeSet(psMSCDevice->sPrivateData.psDMAInstance, psMSCDevice->sPrivateData.ui8OUTDMA, 32);

    USBLibDMAArbSizeSet(psMSCDevice->sPrivateData.psDMAInstance, psMSCDevice->sPrivateData.ui8OUTDMA, 16);
    ...
    if (psMSCDevice->sPrivateData.ui8INDMA != 0) {
        USBLibDMAChannelRelease(psMSCDevice->sPrivateData.psDMAInstance, psMSCDevice->sPrivateData.ui8INDMA);
    }

    //
    // Configure the DMA for the IN endpoint.
    //
    psMSCDevice->sPrivateData.ui8INDMA = USBLibDMAChannelAllocate(
        psMSCDevice->sPrivateData.psDMAInstance, psMSCDevice->sPrivateData.ui8INEndpoint, 64, USB_DMA_EP_TX | USB_DMA_EP_DEVICE);

    USBLibDMAUnitSizeSet(psMSCDevice->sPrivateData.psDMAInstance, psMSCDevice->sPrivateData.ui8INDMA, 32);

    USBLibDMAArbSizeSet(psMSCDevice->sPrivateData.psDMAInstance, psMSCDevice->sPrivateData.ui8INDMA, 16);
...
void *USBDMSCInit(uint32_t ui32Index, tUSBDMSCDevice *psMSCDevice) {
    ...
    //
    USBDCDInit(ui32Index, &psMSCDevice->sPrivateData.sDevInfo, (void *)psMSCDevice);
...
void *USBDMSCCompositeInit(uint32_t ui32Index, tUSBDMSCDevice *psMSCDevice, tCompositeEntry *psCompEntry) {
    ...
    // structure.
    //
    psInst->sDevInfo.ppui8StringDescriptors   = psMSCDevice->ppui8StringDescriptors;
    psInst->sDevInfo.ui32NumStringDescriptors = psMSCDevice->ui32NumStringDescriptors;
...
//
//*****************************************************************************
static void HandleRequests(void *pvMSCDevice, tUSBRequest *pUSBRequest) {
    //
...
//
//*****************************************************************************
static void USBDSCSIInquiry(tUSBDMSCDevice *psMSCDevice) {
    int32_t i32Idx;
    tMSCInstance *psInst;
    uint32_t *pui32Data;
    ...
    //
    MAP_USBEndpointDataPut(USB0_BASE, psInst->ui8INEndpoint, g_pui8Command, 36);
...
//
//*****************************************************************************
static void USBDSCSIReadCapacities(tUSBDMSCDevice *psMSCDevice) {
    uint32_t ui32Blocks;
    tMSCInstance *psInst;
    uint32_t *pui32Data;
    ...
    if (psInst->pvMedia != 0) {
        if (psMSCDevice->sMediaFunctions.pfnBlockSize) {
            ...
            //
            g_pui32BlockSize = psMSCDevice->sMediaFunctions.pfnBlockSize(psInst->pvMedia);
        }
        ...
        //
        MAP_USBEndpointDataPut(USB0_BASE, psInst->ui8INEndpoint, g_pui8Command, 12);
        MAP_USBEndpointDataSend(USB0_BASE, psInst->ui8INEndpoint, USB_TRANS_IN);
        ...
        //
        MAP_USBDevEndpointStall(USB0_BASE, psInst->ui8INEndpoint, USB_EP_DEV_IN);
...
//
//*****************************************************************************
static void USBDSCSIReadCapacity(tUSBDMSCDevice *psMSCDevice) {
    uint32_t ui32Blocks;
    tMSCInstance *psInst;
    ...
    if (psInst->pvMedia != 0) {
        ...
        // Send the SCSI Inquiry Response.
        //
        MAP_USBEndpointDataPut(USB0_BASE, psInst->ui8INEndpoint, g_pui8Command, 8);
        MAP_USBEndpointDataSend(USB0_BASE, psInst->ui8INEndpoint, USB_TRANS_IN);
...
//
//*****************************************************************************
static void USBDSCSIRequestSense(tUSBDMSCDevice *psMSCDevice) {
    tMSCInstance *psInst;
    int32_t i32Idx;
    ...
    // Send the SCSI Inquiry Response.
    //
    MAP_USBEndpointDataPut(USB0_BASE, psInst->ui8INEndpoint, g_pui8Command, 18);
    MAP_USBEndpointDataSend(USB0_BASE, psInst->ui8INEndpoint, USB_TRANS_IN);
...
//
//*****************************************************************************
static void USBDSCSIRead10(tUSBDMSCDevice *psMSCDevice, tMSCCBW *psSCSICBW) {
    ...
    if (psInst->pvMedia != 0) {
        ...
        //
        psInst->ui32CurrentLBA =
            (psSCSICBW->CBWCB[2] << 24) | (psSCSICBW->CBWCB[3] << 16) | (psSCSICBW->CBWCB[4] << 8) | (psSCSICBW->CBWCB[5] << 0);
        ...
        //
        if (psMSCDevice->sMediaFunctions.pfnBlockRead(psInst->pvMedia, (uint8_t *)psInst->pui32Buffer, psInst->ui32CurrentLBA, 1) == 0) {
            psInst->pvMedia = 0;
            psMSCDevice->sMediaFunctions.pfnClose(0);
        }
    }
...
//
//*****************************************************************************
static void USBDSCSIWrite10(tUSBDMSCDevice *psMSCDevice, tMSCCBW *psSCSICBW) {
    uint16_t ui16NumBlocks;
    tMSCInstance *psInst;
    ...
    if (psInst->pvMedia != 0) {
        ...
        // Stall the IN endpoint
        //
        MAP_USBDevEndpointStall(USB0_BASE, psInst->ui8OUTEndpoint, USB_EP_DEV_OUT);
...
//
//*****************************************************************************
static void USBDSCSIModeSense6(tUSBDMSCDevice *psMSCDevice, tMSCCBW *psSCSICBW) {
    tMSCInstance *psInst;
    ...
    if (psInst->pvMedia != 0) {
        ...
        //
        MAP_USBEndpointDataPut(USB0_BASE, psInst->ui8INEndpoint, g_pui8Command, 4);
        MAP_USBEndpointDataSend(USB0_BASE, psInst->ui8INEndpoint, USB_TRANS_IN);
...
static void USBDSCSISendStatus(tUSBDMSCDevice *psMSCDevice) {
    ...
    //
    MAP_USBEndpointDataPut(USB0_BASE, psInst->ui8INEndpoint, (uint8_t *)&g_sSCSICSW, 13);
    MAP_USBEndpointDataSend(USB0_BASE, psInst->ui8INEndpoint, USB_TRANS_IN);
...
//
//*****************************************************************************
static void USBDSCSIPreventAllowMediumRemoval(tUSBDMSCDevice *psMSCDevice, tMSCCBW *psSCSICBW) {
    tMSCInstance *psInst;
...
//
//*****************************************************************************
static void USBDSCSIStartStopUnit(tUSBDMSCDevice *psMSCDevice, tMSCCBW *psSCSICBW) {
    tMSCInstance *psInst;
